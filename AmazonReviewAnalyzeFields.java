...
public class AmazonReviewAnalyzeFields extends Configured implements Tool {
	...
	public int run(String[] args) throws Exception {
		...
		Job job = Job.getInstance(getConf(), "VerifiedReviews");
		job.setJarByClass(VerifiedReviews.class);
	...
	public static class MapReduceMapper extends TableMapper<Text, IntWritable> {
		...
		protected void setup(Context context) {
			...
			rowsProcessed = context.getCounter("VerifiedReviews", "Rows Processed");
    		}
		...
		public void map(ImmutableBytesWritable rowKey, Result value, Context context) throws InterruptedException, IOException {
			try {
                ...
                String reviewTimeString = jsonObject.get("reviewTime").getAsString();
                context.write(new Text(reviewTimeString),one);
                ...
                Date date = null;
                try {
                date = format.parse(reviewTimeString); 
                } catch (Exception e) {}
                //String reviewTimeString = jsonObject.get("reviewTime").getAsString();
                //Date reviewTime = new Date (reviewTimeString);  
                context.write(new Text(date),one);
                String dateString = outputformat.format( date );
                context.write(new Text(dateString),one);
        
                String key = verified + "-" + dateString;        
                context.write(new Text(key),one);
                System.out.println("Verified Reviews by Month is: " + key);
